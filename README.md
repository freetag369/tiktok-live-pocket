...
| 機能 | 内容 |
|---|---|
| バックアップを書き出す | 来店履歴・リスナーメモ・ギフト画像キャッシュ・設定を 1 つの JSON に。iPhone では共有シートから「ファイルに保存」や AirDrop。API キーは「含める」を ON にしたときだけ入る |
| バックアップを読み込む | JSON を選んで取り込み。**統合**(回数の多い方を残す・名前とメモは新しい方)か **置き換え**(今の履歴とメモを捨てる)を選べる。設定も入っていれば反映 |
...
src/components/     Header / FeedList / FeedRow / MemoSheet / ViewersSheet / Settings / Avatar / Badges
scripts/
