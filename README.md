...
| タブ | すべて / コメント / 入室 / ギフト(ギフトタブは画像が大きい) / ハイライト |
...
「ハイライト」タブでは日付を選び、その日にコメント・いいね・フォロー・ギフトをしたメンバーを一覧できます。日付と時刻は日本時間で、同日の複数配信をまとめ、最後にアクションした人から表示します。メンバーを押すとメモ・よみがな入力の下にその日の履歴が表示されます。いいね・ギフトは受信した個数を合計し、連続ギフトは累積数を更新します。

履歴はアーカイブの保存期間・削除に連動します。保存OFF時の新しい記録は再起動すると消えます。いいねは機能追加後に受信した分から記録され、以前のいいねは復元できません。通常フィードにはいいねを流さず、アーカイブの「すべて」、CSV・JSON、アーカイブ付きバックアップには含めます。
