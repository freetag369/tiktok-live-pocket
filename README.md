...
| 行 | アイコン + 名前 + **初見** または **N回目** バッジ。ギフターレベル(バッジ)持ちの入室は **Lv.N** 付きで出ます(TikTok はこの入室を別種のメッセージで送るため、専用に拾っています)。ギフトは画像 + 名前 + ×連打数 + 💎。しきい値(既定 100💎)以上は金枠で大きく |
...
| 機能 | 内容 |
|---|---|
| バックアップを書き出す | 来店履歴・ギフト画像キャッシュ・設定を 1 つの JSON に。iPhone では共有シートから「ファイルに保存」や AirDrop。API キーは「含める」を ON にしたときだけ入る |
| バックアップを読み込む | JSON を選んで取り込み。**統合**(回数の多い方を残す・名前は新しい方)か **置き換え**(今の履歴を捨てる)を選べる。設定も入っていれば反映 |
| 配信ログの書き出し | いま画面にある行(コメント・入室・フォロー・ギフト)を CSV(Excel/Numbers 用・BOM 付き)または JSON に |
